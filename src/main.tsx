...
function App() {
  ...
  function reset() {
    ...
    setSelected(null); setNotice(''); setText(''); setIsError(false); setZoom(1);
  }
  ...
  return <div className="app-shell">
    <header className="site-header"><a className="brand" href="https://everydayai.work" aria-label="Everyday AI home"><img src="/favicon.svg" alt="" width="28" height="28"/><span>everyday<span className="brand-ai">ai</span><span className="brand-divider">/</span><span className="brand-product">feelings</span></span></a><span className="header-note">A little space to check in.</span></header>
    <main className="experience">
      <section className="wheel-section" aria-labelledby="wheel-heading">
        <div className="wheel-heading"><div><span className="eyebrow">THE FEELINGS WHEEL</span><h1 id="wheel-heading">Find the words<br className="mobile-break"/> for what you feel.</h1></div><span className="edition">01 / A MOMENT FOR YOU</span></div>
        <div className="wheel-wrap"><Wheel rotation={rotation} selected={selected} disabled={busy} zoom={zoom} onSelect={emotion => selectEmotion(emotion)} /></div>
        <div className="wheel-controls"><span className="wheel-hint"><span className={`status-dot${spinning ? ' pulsing' : ''}`}/>{spinning ? 'Finding your place on the wheel…' : selected ? emotionPath(selected) : 'Start at the center. Explore outward.'}</span><div className="zoom-controls" aria-label="Wheel zoom"><button type="button" onClick={() => setZoom(value => Math.max(1, +(value - 0.25).toFixed(2)))} disabled={zoom <= 1} aria-label="Zoom out">−</button><output aria-label="Zoom level">{Math.round(zoom * 100)}%</output><button type="button" onClick={() => setZoom(value => Math.min(2, value + 0.25))} disabled={zoom >= 2} aria-label="Zoom in">+</button></div></div>
      ...
      <section className="input-panel" aria-labelledby="input-heading"><div className="step-label"><span>01</span> PAUSE & NOTICE</div><h2 id="input-heading">How are you,<br/><em>really?</em></h2><p className="intro">A word, a thought, a whole messy sentence. Start wherever you are.</p><form onSubmit={submit}><label htmlFor="feeling-input">What’s on your mind?</label><div className="textarea-wrap"><textarea id="feeling-input" value={text} onChange={event => setText(event.target.value)} maxLength={1000} placeholder="I’ve had so much to do today, and I don’t know where to start…" aria-describedby="input-help privacy-note"/><span className="character-count">{text.length} / 1000</span></div><p id="input-help" className="input-help">English, हिंदी, or Hinglish. All welcome.</p><button className="submit-button" disabled={busy || !text.trim()} type="submit">{loading ? <><span className="spinner"/> Listening to your words…</> : spinning ? 'Turning toward your feeling…' : <>Find my feeling <span aria-hidden="true">↗</span></>}</button><p className="privacy-note" id="privacy-note"><svg width="13" height="15" viewBox="0 0 16 18" fill="none" aria-hidden="true"><rect x="3" y="8" width="10" height="8" rx="2" stroke="currentColor"/><path d="M5 8V5a3 3 0 0 1 6 0v3" stroke="currentColor"/></svg>Your words are sent to Groq for a match. This site doesn’t save them.</p></form><div className={`notice${isError ? ' error' : ''}`} role="status" aria-live="polite">{notice}</div></section>
      <section className={`guidance-panel${selected ? ' has-selection' : ''}`} aria-labelledby="guidance-heading" aria-live="polite" aria-atomic="true">
        ...
        {selected ? <><div className="result-heading"><div><p className="match-label">{source === 'ai' ? 'A possible match' : 'You’re exploring'}</p><h2 id="guidance-heading">{selected.label}</h2><span lang="hi" className="hindi-title">{selected.hindi}</span></div><span className="emotion-swatch" style={{ background: roots.find(root => root.id === selected.family)?.color }} aria-hidden="true"/></div><p className="emotion-description">{selected.description}</p><div className="examples"><h3>It might sound like</h3>{selected.examples.map(([english, hindi], index) => <div className="example" key={index}><p>“{english}”</p><p lang="hi">{hindi}</p></div>)}</div><div className="care-block"><h3>How to recognize it</h3><p>{selected.recognize}</p></div><div className="care-block"><h3>A little way forward</h3><ul>{selected.actions.map(action => <li key={action}>{action}</li>)}</ul></div><p className="gentle-note">You know your experience best. If this doesn’t fit, try another feeling.</p><button type="button" className="reset-button" onClick={reset}>Start a fresh check-in <span aria-hidden="true">↺</span></button></> : <div className="empty-guidance"><div className="petal-mark" aria-hidden="true">✳</div><h3 id="guidance-heading">Every feeling has a place.</h3><p>Your match will appear here, with English and Hindi examples and a few gentle ways to care for yourself.</p><span>No right answers. Just a starting point.</span></div>}
      </section>
    </main>
